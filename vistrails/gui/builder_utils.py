""" Utilities for the Vistrail Builder: points, rect regions, etc.
...
class ColorByName(object):
    ...
    @staticmethod
    def get(name):
        """get(str) -> Color. Returns a color with the given name."""
        return ColorByName.colors[name] + [1.0]

import xml.sax.saxutils
def toHtml(text):
    """toHtml(text) - PCDATA. Escape html pcdata."""
    return xml.sax.saxutils.escape(text)

def adjustMousePressButton(event):
    """adjustMousePressButton(QtCore.QMouseEvent) -> QtCore.QMouseEvent.
    Corrects mouse event state so that Shift+Click equals Middle Click and
    Ctrl+Click equals Right Click."""
    st = event.buttons()
    b = event.modifiers()
