  ...
  if os.path.isfile(pathfile):
      ...
      for w in wsdlList:
          ...
          try:
              ...
              pm.show_error_message(pm.get_package_by_identifier(identifier),"Malformed URL: %s" % w)
              continue
          location = w
      ...
      for w in wsdlList:
          ...
          for element in wsdlList:
              if w == element.strip():
                  numtimes = numtimes + 1
          if numtimes > 1:
              pm.show_error_message(pm.get_package_by_identifier(identifier),"The following web service is repeated in the list: %s"%w)
              continue
          try:
              s = w.split('/')
              host = s[2]
