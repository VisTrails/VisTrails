...
def translateVistrail(_vistrail):
    ...
    def update_tags(old_obj, translate_dict):
        new_tags = []
        for (id, tag) in tag_annotations.iteritems():
            new_tags.append(DBTag(id=id,
                                  name=tag))
        return new_tags

    def update_prune(old_obj, translate_dict):
        if old_obj.db_id in prune_annotations:
            prune_val = prune_annotations[old_obj.db_id]
    ...
    def update_annotations(old_obj, translate_dict):
        ...
        if old_obj.db_id in notes_annotations:
            notes = notes_annotations[old_obj.db_id]
            new_annotations.append(
                DBAnnotation(id=id_scope.getNewId(DBAnnotation.vtType),
                             key='__notes__',
                             value=notes))
        if old_obj.db_id in upgrade_annotations:
            upgrade = upgrade_annotations[old_obj.db_id]
            new_annotations.append(
                DBAnnotation(id=id_scope.getNewId(DBAnnotation.vtType),
                             key='__upgrade__',
                             value=upgrade))
        if old_obj.db_id in thumb_annotations:
            thumb = thumb_annotations[old_obj.db_id]
            new_annotations.append(
                DBAnnotation(id=id_scope.getNewId(DBAnnotation.vtType),
                             key='__thumb__',
                             value=thumb))        
    ...
    for a in _vistrail.db_actionAnnotations:
        if a.db_key in key_lists:
            key_lists[a.db_key][a.db_action_id] = a.db_value
